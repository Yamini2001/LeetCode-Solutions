...
class Solution {
    ...
    private String justify(String[] words, int i, int j, int maxWidth) {
        ...
        int numWordsToPad = j - i; // for total of 3 words, j=2, i=0 so 2 words to pad (since we don't pad last
                                   // one)

        StringBuilder sb = new StringBuilder();

        // SpaceStr is the string with right num of spaces that should be attached to
        // each word (numWordsToPad)
        String spaceStr = isLastLine ? " " : blank(numSpaces / numWordsToPad); // simple separation in last line
        // remainderSpaceCount is the number of extra space that need to be attached to
        // first words (from left)
        int remainderSpaceCount = isLastLine ? 0 : numSpaces % numWordsToPad;

        int usedTotal = 0;
        for (int k = i; k <= j; k++) {
            sb.append(words[k]).append(spaceStr); // notice we also end up attaching to the last word, we will trim it
                                                  // later
